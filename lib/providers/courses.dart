...
class Courses with ChangeNotifier {
  ...
  List<Course> _certificateItems = [];
  List<Course> _jobGuaranteeItems = [];
  List<CourseDetail> _courseDetailsitems = [];

  Courses(this._items, this._certificateItems,this._jobGuaranteeItems);
  ...
  List<Course> get jCourseItems {
    return [..._jobGuaranteeItems];

  } List<Course> get cCourseItems {
    return [..._certificateItems];
  }
  ...
  Future<void> fetchTopCourses() async {
    ...
    // read userId from SharedPrefs
    final userId = await SharedPreferenceHelper().getUserId();
    final uid = (userId == null || userId.trim().isEmpty) ? '0' : userId.trim();

    var url = '$BASE_URL/api/top_courses?user_id=$uid';
    debugPrint('[TopCourses] GET $url');
  ...
  Future<void> fetchCertificateCourses() async {
    // read userId from SharedPrefs
    final userId = await SharedPreferenceHelper().getUserId();
    final uid = (userId == null || userId.trim().isEmpty) ? '0' : userId.trim();

    var url = '$BASE_URL/api/program_type_wise_courses/certificate?user_id=$uid';
    debugPrint('[Certificate Course] GET $url');
    ...
    try {
      ...
      // print(extractedData);
      _certificateItems = buildCourseList(extractedData);
      notifyListeners();
    } catch (error) {
      rethrow;
    }
  }
  Future<void> fetchJobGuaranteeCourses() async {
    // read userId from SharedPrefs
    final userId = await SharedPreferenceHelper().getUserId();
    final uid = (userId == null || userId.trim().isEmpty) ? '0' : userId.trim();

    var url = '$BASE_URL/api/program_type_wise_courses/job-guarantee?user_id=$uid';
    debugPrint('[JobGuarantee] GET $url');
    ...
    try {
      ...
      // print(extractedData);
      _jobGuaranteeItems = buildCourseList(extractedData);
      notifyListeners();
    } catch (error) {
      rethrow;
    }
  ...
  Future<void> fetchCoursesByCategory(int categoryId) async {
    ...
    var url = '$BASE_URL/api/category_wise_course?category_id=$categoryId&user_id=$uid';
    debugPrint('[Categorywise course] GET $url');
  ...
  Future<void> fetchCoursesBySearchQuery(String searchQuery) async {
    ...
    final uid = (userId == null || userId.trim().isEmpty) ? '0' : userId.trim();
    var url =
        '$BASE_URL/api/courses_by_search_string?search_string=$searchQuery&user_id=$uid';
      debugPrint('[Search Courses ] GET $url');
  ...
  Future<void> filterCourses(
    ...
    final uid = (userId == null || userId.trim().isEmpty) ? '0' : userId.trim();
    var url =
        '$BASE_URL/api/filter_course?selected_category=$selectedCategory&selected_price=$selectedPrice&selected_level=$selectedLevel&selected_language=$selectedLanguage&selected_rating=$selectedRating&selected_search_string=&user_id=$uid';
    debugPrint('[FilterCourse] GET $url');
  ...
  List<Course> buildCourseList(List extractedData) {
    ...
    for (var courseData in extractedData) {
      ...
      print(courseData['video_url']);
      print(courseData['course_overview_provider']);
    }
  ...
  List<Lesson> buildCourseLessons(List extractedLessons) {
    ...
    for (var lessonData in extractedLessons) {
      loadedLessons.add(Lesson(
        ...
        videoUrlWeb: lessonData['video_url_web'].toString(),

...
class MyCourses with ChangeNotifier {
  ...
  List<MyCourse> buildMyCourseList(List extractedData) {
    ...
    for (final courseData in extractedData) {
      loadedCourses.add(MyCourse(
        id: _asInt(courseData['id']),
        title: _asString(courseData['title']),
        thumbnail: _asString(courseData['thumbnail']),
        price: _asString(courseData['price']),
        instructor: _asString(courseData['instructor_name']),
        rating: _asInt(courseData['rating']),
        totalNumberRating: _asInt(courseData['number_of_ratings']),
        numberOfEnrollment: _asInt(courseData['total_enrollment']),
        shareableLink: _asString(courseData['shareable_link']),
        courseOverviewProvider: _asString(courseData['course_overview_provider']),
        courseOverviewUrl: _asString(courseData['video_url']),
        courseCompletion: _asInt(courseData['completion']),
        totalNumberOfLessons: _asInt(courseData['total_number_of_lessons']),
        totalNumberOfCompletedLessons: _asInt(courseData['total_number_of_completed_lessons']),
        enableDripContent: _asString(courseData['enable_drip_content']),
        language: _asString(courseData['language']),
      ));
    }
  ...
  Future<void> fetchCourseSections(int courseId,String lang) async {
    _sectionItems=[];
    final authToken = await SharedPreferenceHelper().getAuthToken();
    var url =
        '$BASE_URL/api/sections?auth_token=$authToken&course_id=$courseId&language=$lang';
  ...
  Future<List<Lesson>> fetchLessonDetails(int lessonId, String lang) async {
    final authToken = await SharedPreferenceHelper().getAuthToken();
    var url =
        '$BASE_URL/api/lesson_details?auth_token=$authToken&lesson_id=$lessonId&language=$lang';
    ...
    try {
      ...
      for (var lessonData in extractedData) {
        loadedLessons.add(Lesson(
          ...
          attachmentUrl: lessonData['thumbnail'],
          isCompleted: lessonData['quiz_attempt'].toString(),
          videoUrlWeb: lessonData['video_url_web'].toString()?? "",
          videoTypeWeb: lessonData['video_type_for_mobile_application'] ?? "",
          vimeoVideoId: lessonData['cloud_video_id'],
        ));
      }
